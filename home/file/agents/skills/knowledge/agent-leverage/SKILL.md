...
Each moat from **professional-differentiation** maps to concrete infrastructure in the skill ecosystem:

| Moat | Operationalized through | How |
|------|------------------------|-----|
| Taste | **decision-priorities**, **design-lenses**, **code-review** | Standards encoded as reusable evaluation criteria that apply automatically |
| Domain knowledge | **client-quality-focus**, codebase-specific skills, **work-item-context** | Business rules and patterns captured for consistent application across sessions |
| Ownership | Approval gates in **external-communications**, verification steps in workflows | The user's seal required at decision points; nothing ships without certification |
| Systems thinking | **architect-thinking**, **building-microservices** | Architectural principles applied automatically to every plan and review |
| Deep work | **context-engineering**, strategic delegation | Agent handles mechanical work, freeing sustained focus for genuinely hard problems |
...
|--------|-----------------|----------------------|
| PR description | Motivates the reviewer, structures the walkthrough, scopes precisely | **objective-communication** |
| Code review | Demonstrates architectural judgment, domain awareness, proportionate feedback | **code-review** + **architect-thinking** |
| Commit | Tells a clean atomic story; imperative mood, explains why not what | **commit-conventions** |
| Slack message | Crisp, concrete, valuable; respects the reader's time | **external-communications** |
| Plan | Grounded in prior art and codebase evidence; structured for decision-making | **prior-art-research** + **plan** |
| Bug report | Concrete reproduction steps, correct severity, necessary context | **write-repro-steps** |
| Estimate | Calibrated to codebase evidence, explicit about assumptions and risk | **estimation** |
...
**external-communications** requires explicit approval before posting to any external platform. This is not bureaucracy -- it is the moment you exercise ownership. Read the candidate text as a stranger would. Ask: does this represent my professional judgment?
...
**continuous-improvement** proposes refinements after each execution. **retrospective** identifies patterns across sessions. YOUR observations -- what you accept, reject, or modify -- become encoded standards. Every refinement raises the baseline for all future work.
...
3. **Proposal** -- **continuous-improvement** suggests a specific refinement
...
The **retrospective** skill runs this loop at batch scale (weekly, across all sessions). The compounding effect: each week's baseline is higher than the last. Over months, the gap between your agent-augmented output and generic AI output becomes unbridgeable.
...
- **professional-differentiation** -- defines the moats; this skill operationalizes them through the agent system.
- **continuous-improvement** -- the reactive feedback mechanism that proposes system evolution after each execution.
- **retrospective** -- the proactive batch-level mechanism that identifies patterns across sessions.
- **context-engineering** -- enables deep work by managing cognitive load; keeps the agent efficient.
- **external-communications** -- the outward-facing quality gate where reputation is built or eroded.
- **objective-communication** -- the intellectual standards for all delivered text.
- **decision-priorities** -- the priority ladder that encodes taste into every design decision.
