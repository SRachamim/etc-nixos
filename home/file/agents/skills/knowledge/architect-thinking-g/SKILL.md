...
- In microservice contexts, **independent deployability** is the primary option to preserve -- it enables all other options (technology choice, scaling, team autonomy). See the **microservice-patterns-g** skill for microservice-specific options analysis.
...
- In microservice contexts, service boundaries should align with different rates of change -- Newman's guidance to extract volatile code into separate services is a direct application of multispeed architecture. See the **microservice-patterns-g** skill for decomposition drivers.
...
- Distributed microservice systems exhibit characteristic feedback loops: retry storms, cascading failures, circuit breaker dynamics, and timeout amplification. See the **microservice-patterns-g** skill Resiliency section for stability patterns that manage these loops.
...
- Conway's law is the primary driver of microservice boundary design -- loosely coupled organisations produce modular systems. See the **microservice-patterns-g** skill Organisation and Architecture section for team structure, strong ownership, and the paved-road platform model.
