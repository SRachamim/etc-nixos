...
- **Design** -- Is the abstraction level appropriate? Are responsibilities well-separated? When the PR introduces a new abstraction (component, hook, module, utility), search the codebase for existing abstractions that solve the same problem -- can they be reused, composed, or generalized instead of reimplemented? A parallel implementation of an existing pattern is a design finding even when the new version is well-written.
...
Apply the **commit-conventions-g** skill to evaluate whether commits are well-structured, independently valid, and tell a coherent story.