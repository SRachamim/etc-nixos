...
Evaluates structural transformations through the **refactoring-g** skill's core principles.
...
For each structural change, select techniques from the **refactoring-g** skill catalog. Reference each technique by its catalog name (e.g. Extract Module, Move Function, Introduce Branded Type). Apply the **functional-typescript-g** skill to ensure the target structure aligns with fp-ts standards and architectural principles.
...
| Does it name a recognised technique from the **refactoring-g** skill? | Technique column is blank or vague |
...
Evaluates significant design choices through principles from the **architect-thinking-g** skill.
