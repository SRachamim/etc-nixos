---
name: fgrepo-artifact-precedence-g
...
This skill is a repo-specific specialization of the **artifact-layering-g** model. It adds fgrepo-specific directory-boundary scoping on top of the general layering framework.
...
- A repo-root `.cursor/rules/` file prescribes a coding convention that contradicts the user's **functional-typescript-g** skill (e.g., mandating classes and inheritance).
- A repo-level skill dictates commit message conventions that differ from the user's **commit-conventions-g** skill.
- A repo-level rule enforces a review workflow that conflicts with the user's **review-pr-g** command or **code-review-g** skill.
