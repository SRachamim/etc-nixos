...
7. **Architecture Sells Options** -- Prefer designs that preserve future options. Use discriminated unions (open for new variants), `Reader` (deferred configuration), and `Either` (reversible choices) to avoid locking in decisions. When uncertainty is high, the value of keeping options open increases. Evaluate the "strike price" of each option: what does it cost to keep the option available vs. committing now?
8. **Configuration as Code** -- Treat configuration as a first-class citizen: validate with `io-ts` codecs, version-control alongside source, use branded types for config values (e.g., `Port`, `ConnectionString`), and test configuration parsing. "Configuration" that contains conditional logic is programming in a poorly designed language -- promote it to proper TypeScript.
...
For the full test-driven development workflow -- the Red/Green/Refactor cycle, strategies for getting to green (Fake It, Triangulation, Obvious Implementation), test list management, property-based testing patterns, and fp-ts-specific testing patterns -- see the **test-driven-development-g** skill.

## Code Smells

Recognize these smells during reviews or while working in a codebase. The "Primary Remedies" column references the **refactoring-g** skill's catalog.
