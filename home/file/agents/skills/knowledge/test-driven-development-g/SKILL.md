---
name: test-driven-development-g
...
The **functional-typescript-g** skill's algebra/interpreter separation provides the natural mock boundary. Define a test interpreter that returns canned values from `E.right(...)` or `TE.right(...)`. No mocking library needed.
...
| Skill | Relationship |
|---|---|
| **functional-typescript-g** | Defines *what good code looks like*: types, patterns, architecture. TDD drives you toward that code through failing tests. |
| **refactoring-g** | Defines *how to transform code*. Refactoring is step 3 of the TDD cycle. Every refactoring happens under a green bar. |
