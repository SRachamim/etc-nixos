...
Require **Ask** mode following the **mode-gate-g** skill. This command is primarily informational -- it researches a Slack conversation and presents findings without making changes. If the user later wants to post a Slack reply (step 7), they should switch to Agent mode for that.
...
| Azure DevOps pull request (`_git/<repo>/pullrequest/<id>`) | Extract the ID and fetch via `repo_get_pull_request_by_id` |
...
|--------|-------------------------------|
| Code questions | Search the codebase; apply the **functional-typescript-g** skill for TypeScript questions |
| Incidents | Follow the **incident-response-g** skill's investigation structure; query Datadog via MCP |
| Build failures | Follow the **triage-build-g** skill's investigation pattern; query Azure DevOps and Currents via MCP |
