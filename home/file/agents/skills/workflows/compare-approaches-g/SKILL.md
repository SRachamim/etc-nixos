...
2. **Decision point from `/plan-g` or `/analyze-prd-g`** -- when step 5 (design lenses) or step 6 (tech design) reveals multiple viable paths. The calling skill passes the decision context.
...
Require **Plan** mode following the **mode-gate-g** skill. The entire comparison (steps 1--6) is read-only analysis.
...
Apply the **decision-priorities-g** skill and the **design-lenses-g** skill (planning framing) across all candidates:
...
- If invoked from `/plan-g`: return the chosen approach as the design input for step 6 (draft the plan).
