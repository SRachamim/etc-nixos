---
name: debug-g
description: Investigates bugs to find root cause, designs a TDD-based fix plan, and implements after approval. Use when given a bug ticket or description of defective behaviour that needs investigation and fixing in the codebase.
...
Require **Debug** mode following the **mode-gate-g** skill. This command investigates a bug in the codebase -- Debug mode is the best fit for the investigation phase (steps 1--4).
...
If the bug is visual, UI-driven, or requires interacting with the running application to reproduce, apply the **browser-bug-reproduction-g** skill to verify the behaviour in the browser before proceeding to root-cause analysis.
...
Require **Plan** mode following the **mode-gate-g** skill. The fix-planning phase (steps 6--8) is read-only design -- Plan mode keeps the focus on discussion rather than edits.

### 6. Design the fix

Design a sequence of commits to fix the bug. The **test-driven-development-g** skill's Regression Test pattern is mandatory: the first commit must be a failing test that captures the bug.
...
2. **Preparatory refactoring** (if needed) -- when the fix requires structural changes, apply the **refactoring-g** skill's techniques. Each refactoring commit is behaviour-preserving and independent.
3. **Fix** -- the minimal code change that makes the regression test pass.

#### Refactoring lens

When the fix involves restructuring, evaluate through the **refactoring-g** skill's core principles:
...
For each structural change, select techniques from the **refactoring-g** skill catalogue. Apply the **functional-typescript-g** skill to ensure the fix aligns with fp-ts standards.

Design commits following the **commit-conventions-g** skill.

### 7. Present the fix plan

Apply the **objective-communication-g** skill to all plan text.
...
Once the user approves, implement the fix plan **in the exact sequence presented**. Build the TODO list and execute each item following the **plan-execution-g** skill.
