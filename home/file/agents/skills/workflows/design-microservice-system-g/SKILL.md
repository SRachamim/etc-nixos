...
Require **Plan** mode following the **mode-gate-g** skill. Steps 1–13 are read-only analysis and design — Plan mode keeps the focus on discussion rather than premature implementation. The user will switch to Agent mode separately when they want to build.
...
- Apply the **architect-thinking-g** skill, section: **Communication** (build ramps not cliffs, show the pirate ship, emphasis over completeness, five-second test, writing for busy people).
