...
Require **Plan** mode following the **mode-gate-g** skill. Steps 1–10 are read-only analysis and planning. The user will switch to Agent mode at step 11 for implementation.
...
- Apply **commit-conventions-g** and **objective-communication-g** skills to all plan text.
...
- Require **Agent** mode following the **mode-gate-g** skill. This is a mid-command mode transition — apply the full gate protocol (attempt, verify, stop if not active).
- Build the TODO list from the extraction plan following the **plan-execution-g** skill:
