---
name: list-closeable-worktrees-g
description: Scans local feature branches and checks Azure DevOps for completed PRs and resolved work items. Use when you need to identify features ready to close via /close-worktree.
disable-model-invocation: true
---

# List Closeable Worktrees

Scan all local `feature/*` branches, check Azure DevOps for completed PRs and resolved work items, and present a summary of features ready to be closed via `/close-worktree`.
...
If there are closeable features, suggest running `/close-worktree <id>` for each one. If a closeable feature's work item is not in **Resolved** state, flag it with a warning.
