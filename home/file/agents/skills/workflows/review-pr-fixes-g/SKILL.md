...
- **Fixed / Closed**: verify the code at that location actually addresses the original concern. If adequate, mark for **resolution** (status -> `Fixed`). If not, draft a follow-up explaining what's still missing and mark for **reactivation** (status -> `Active`).
  When the original finding asked for something to be *removed* (formatting churn, dead code, a suppression, debug output), verify against the merge base, not the delta -- the delta shows the removal as another edit in the same region, which reads the same whether it fixed the problem or repeated it. `git diff -w --stat origin/<target>...origin/<source> -- <file>` should report only the intended change; for formatting churn, `git diff origin/<target>...origin/<source> -- <file> | grep -c '^[+-]$'` should report zero.
...
- **fgrepo client/ quality gate**: When the PR belongs to fgrepo and the delta includes changed files under `client/`, read the repo-level `client-code-quality-gate` skill (`client/.cursor/skills/development/client-code-quality-gate/SKILL.md`) and apply its verification checks (Sections A--C) to each changed client/ file during delta evaluation. Quality gate checks are conditional on each check's "When to run" trigger -- skip checks whose trigger does not apply. Map quality gate severity to review findings: `blocking` items become blocking review findings, `warning` items become suggestions. A single check can bundle sub-conditions of different gravity -- map on the sub-condition that actually failed, not the check's headline severity. When the mapped severity differs from the headline, state the deviation and its reasoning in the finding so the severity is reproducible. This loading is mandatory -- do not rely on path-based auto-discovery.
