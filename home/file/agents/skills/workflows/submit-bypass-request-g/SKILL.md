---
name: submit-bypass-request-g
description: Composes and posts a pipeline bypass request to the #pipeline-gated Slack channel for a failed PR build. Use after triage-build or when a PR pipeline failed and you need approval to bypass the gate.
...
This command is often invoked after **triage-build-g** in the same conversation. When prior triage context is available (build ID, pipeline URL, test failures, commit summary), reuse it -- don't re-fetch from APIs. Steps below note where conversation context can substitute for a fresh lookup.
...
The **external-communications-g** skill governs all of these, but they're critical for this channel -- reinforce them here:
...
```json
[
  {"type": "section", "text": {"type": "mrkdwn", "text": "Submitted by: <@ID>"}},
  {"type": "section", "text": {"type": "mrkdwn", "text": "*PR Link:*\n<URL|PR #NNNNN>"}},
  {"type": "section", "text": {"type": "mrkdwn", "text": "*Pipeline link*\n<URL>"}},
  {"type": "section", "text": {"type": "mrkdwn", "text": "*Which branch am I rebased on?*\n<BRANCH>"}},
  {"type": "section", "text": {"type": "mrkdwn", "text": "*Roughly, what did I change in the PR?*\n<CHANGES>"}},
  {"type": "section", "text": {"type": "mrkdwn", "text": "*What tests failed, and why do I think it isn't due to my changes?*\n<EXPLANATION>"}},
  {"type": "section", "text": {"type": "mrkdwn", "text": "*Ping the relevant stability owner*\n<@STABILITY_OWNER_ID>"}}
]
```
