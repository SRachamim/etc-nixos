...
Unlike the **triage-transition-g** skill (which performs a mechanical estimate-and-transition), this command adds investigation and fix-planning phases so the triage decision is informed by codebase evidence.
...
Require **Plan** mode following the **mode-gate-g** skill. All investigation, analysis, and drafting (steps 1--5) are read-only. The user will switch to Agent mode for the apply step.

### 1. Observe

Apply the **work-item-context-g** skill to build the full picture:
...
2. Recommend delegating to the **block-work-item-g** skill, passing the current work item as the blocked ID and the predecessor as the blocker ID.
...
Once the user approves, require **Agent** mode following the **mode-gate-g** skill, then apply the triage updates via `update_work_item` following the **triage-transition-g** skill's update format (step 4 of that skill).
